...
INITIAL_PERMUTATION = [57, 49, 41, 33, 25, 17, 9, 1, 58, 50, 42, 34, 26, 18,
                       10, 2, 59, 51, 43, 35, 27, 19, 11, 3, 60, 52, 44, 36,
                       63, 55, 47, 39, 31, 23, 15, 7, 62, 54, 46, 38, 30, 22,
                       14, 6, 61, 53, 45, 37, 29, 21, 13, 5, 28, 20, 12, 4]

FINAL_PERMUTATION = [39, 7, 47, 15, 55, 23, 63, 31, 38, 6, 46, 14, 54, 22, 62, 30,
                     37, 5, 45, 13, 53, 21, 61, 29, 36, 4, 44, 12, 52, 20, 60, 28,
                     35, 3, 43, 11, 51, 19, 59, 27, 34, 2, 42, 10, 50, 18, 58, 26,
                     33, 1, 41, 9, 49, 17, 57, 25, 32, 0, 40, 8, 48, 16, 56, 24]

EXPANSION_TABLE = [31, 0, 1, 2, 3, 4, 3, 4, 5, 6, 7, 8, 7, 8, 9, 10, 11, 12, 11, 12, 13, 14, 15, 16,
                   15, 16, 17, 18, 19, 20, 19, 20, 21, 22, 23, 24, 23, 24, 25, 26, 27, 28, 27, 28, 29, 30, 31, 0]

PERMUTATION_FUNCTION = [15, 6, 19, 20, 28, 11, 27, 16, 0, 14, 22, 25,
                        4, 17, 30, 9, 1, 7, 23, 13, 31, 26, 2, 8, 18, 12, 29, 5, 21, 10, 3, 24]

PC_1 = [56, 48, 40, 32, 24, 16, 8, 0, 57, 49, 41, 33, 25, 17, 9, 1, 58, 50, 42, 34, 26, 18, 10, 2, 59, 51, 43, 35, 62,
        54, 46, 38,
        30, 22, 14, 6, 61, 53, 45, 37, 29, 21, 13, 5, 60, 52, 44, 36, 28, 20, 12, 4, 27, 19, 11, 3]

PC_2 = [13, 16, 10, 23, 0, 4, 2, 27, 14, 5, 20, 9, 22, 18, 11, 3, 25, 7, 15, 6, 26, 19, 12, 1,
...
def permute(block, table):
    if block.size < max(table):
        raise ValueError("Permutation index out of range for the given block size")
...
def substitute(expanded_half_block):
    ...
    for sindex in range(len(segments)):
        ...
        output[sindex * 4:sindex * 4 + 4] = BitVector(intVal=S_BOX[sindex][row][column], size=4)
    return output
...
def generate_round_keys(key):
    ...
    for shift in SHIFT_TABLE:
        ...
        round_key = permute(round_key, PC_2)
...
def des_round(left, right, round_key):
    ...
    permuted = permute(substituted, PERMUTATION_FUNCTION)
    new_right = left ^ permuted
    return right, new_right


def des_encrypt(plain_text, round_keys):
    block = BitVector(textstring=plain_text)
    if block.length() != 64:
        raise ValueError("Plaintext must be exactly 8 characters long (64 bits).")
...
def main():
    ...
    user_key = getpass.getpass(prompt="Enter an 8 character encryption key: ")
    if len(user_key) != 8:
        raise ValueError("Key must be exactly 8 characters long")
    key = BitVector(textstring=user_key)

    # Generate round keys
    round_keys = generate_round_keys(key)

    # Encrypt
    plain_text = input("Enter the plaintext to encrypt (exactly 8 characters): ")
    if len(plain_text) != 8:
        raise ValueError("Plaintext must be exactly 8 characters long")
    cipher_text = des_encrypt(plain_text, round_keys)
    print("Ciphertext:", cipher_text.get_bitvector_in_hex())

    # Decrypt
    decrypted_text = des_decrypt(cipher_text, round_keys)
    print("Decrypted text:", decrypted_text.get_bitvector_in_ascii())
